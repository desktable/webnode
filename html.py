...
class StartTag(Raw):
    ...
    def __init__(self, tag, attrs=None):
        self.tag = tag
        if attrs:
            self.raw = '<%s %s>' % (tag, ' '.join(self._attr_from_kv(*kv)
                                                  for kv in attrs.iteritems()))
        else:
            self.raw = '<%s>' % tag

    def _attr_from_kv(self, keyword, value):
        attribute = keyword.rstrip('_').replace('_', '-')
        if value is None:
            return attribute
...
class button(NormalHTMLElement): pass
...
class footer(NormalHTMLElement): pass
...
class header(NormalHTMLElement): pass
...
class nav(NormalHTMLElement): pass
